...
switch ($_GET['accion']) {
    case 'agregar':
        ...
        $respuesta = mysqli_query($conexion, "select precio from productos where
        codigo=" . $_POST['codigoproducto']);
        $producto = mysqli_fetch_array($respuesta);

        $respuesta = mysqli_query($conexion, "insert into
        detallefactura(codigofactura,codigoproducto,cantidad,precio) values
        ($_GET[codigofactura],$_POST[codigoproducto],$_POST[cantidad],$producto[precio])");
    ...
    case 'confirmardescartarfactura':
        $respuesta = mysqli_query($conexion, "delete from facturas where
        codigo=$_GET[codigofactura]");
        $respuesta = mysqli_query($conexion, "delete from detallefactura where
